...
my $dbfolder = "$inputName\_debarcode_result";
...
sub infer_barcode {
	...
	for (my $l = 0; $l < $maxl; $l++) {
		foreach my $bcseq (sort {$bc1->{$a}{bc} cmp $bc1->{$b}{bc}} keys %{$bc1}) {
			...
			print $outLog "\nL = $l\n";
			print $outLog "seq           = $LCY$seq$N\n";
			print $outLog "bcseq         = $LGN$bcseq$N\n";
			print $outLog "bcseqchunk    = $LGN$bcseqchunk$N\n";
			print $outLog "bcseqchunkrev = $LGN$bcseqchunkrev$N\n";
