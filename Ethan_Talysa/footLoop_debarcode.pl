...
while (my $line = <$in1>) { chomp($line);
	...
	my ($bc, $bcs, $seq2, $qua2) = infer_barcode(\%bc, $seq, $qua, $readName);
...
sub infer_barcode {
	...
	for (my $l = 0; $l < 4; $l++) {
		foreach my $bcseq2 (sort {$bc->{$a}{bc} cmp $bc->{$b}{bc}} keys %bc) {
			...
			if ($seq =~ /($bcseq|$bcseqrev)/) {# or $seq =~ /($bcseq|$bcseqrev)/)) {
#				if (defined $bc->{$bcseq2}{bc2}) {
				...
				if (defined $barcode) {
					$bad{$name} = 1;
					$seq2 = $bcseq;
					print $outLog "$LCY$name\tDBL\t$pos/$seq_length\tprev=$barcode\tcurr=$bc->{$bcseq2}{bc}\tprev=$seq1\tcurr=$seq2$N\n";
				}
				else {
		...
		if (defined $barcode and not defined $bad{$name}) {
			...
			if ($seq =~ /($bcseq|$bcseqrev)/) {
				($bad1, $good1) = $seq =~ /($bcseq|$bcseqrev)/;
			}
			#else {
			#	($good2, $bad2) = $seq =~ /($bcseq|$bcseqrev)/;
			#}
			if (defined $bad1) {
				my $len = length($bad1);
				$seq =~ s/^.{$len}//;
