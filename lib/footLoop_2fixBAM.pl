...
my ($footLoop_2fixBAM_outDir) = $opt_o;
...
#DIELOG($tempLogOut, "\nTEMPLOG:\n$tempLog\n$footLoop_folder: footLoop_2fixBAM.pl: $usage") unless ex([$opt_s,$opt_S,$opt_i,$opt_g]) == 1 or ex($opt_n) == 1 and -e $tempLog;
#DIELOG($tempLogOut, "\nTEMPLOG:\n$tempLog\n$footLoop_folder: footLoop_2fixBAM.pl: please define output (-o)\n") if not defined $opt_o and -e $tempLog;
(print "\nfootLoop_2fixBAM.pl: $usage\n" and exit) unless ex([$opt_s,$opt_S,$opt_i,$opt_g]) == 1 or ex($opt_n) == 1;
(print "\nfootLoop_2fixBAM.pl: please define output (-o)\n" and exit) if not defined $opt_o;
makedir($footLoop_2fixBAM_outDir);
...
my $footLoop_2fixBAM_logFile = "$footLoop_folder/footLoop_2fixBAM_logFile.txt";
open (my $outLog, ">", $footLoop_2fixBAM_logFile) or die "Failed to write to footLoop_2fixBAM_logFile: $!\n";
LOG($outLog, date() . "Logfile = $LRD$footLoop_logFile$N\n"); 

# parse footLoop logfile
my ($BAMFile, $seqFile, $genez) = parse_footLoop_logFile($footLoop_logFile, $outLog); 

# check BAM file
LOG($outLog, date() . "Checking BAM File =$LCY$BAMFile$N=\n");
...
my $debugFile = "$footLoop_2fixBAM_outDir/debug.txt";
my $outBAMfile = "$footLoop_2fixBAM_outDir/$BAMName.fixed";
...
while (my $line = <$in1>) {
	...
	my ($ref2, $seq2, $poz, $seqborder0, $seqborder1) = parse_BAMFile($line, \@ref1);
	my %poz = %{$poz};
	my @seq1 = split("", $seqs);
...
sub check_file {
	...
	DIELOG($outLog, "footLoop_2fixBAM.pl: $type file $file does not exist!\n") if ex($file) == 0;
	DIELOG($outLog, "footLoop_2fixBAM.pl: $type file $file is empty!\n")       if -s $file  == 0;
	...
	if ($file =~ /\.(rmdup|bam)$/) {
		...
		open ($checkfileIn, "samtools view $file|") or DIELOG($outLog, "footLoop_2fixBAM.pl: Failed to read from filetype=$filetype, file=$LCY$file$N: $!\n");
	}
	elsif ($file =~ /\.gz$/ or ($filetype =~ /(gzip|binary)/ and $file !~ /\.(rmdup|bam)$/)) {
		($total_line) = `zcat < $file| wc -l` =~ /^\s*(\d+)/;
		open ($checkfileIn, "zcat < $file|") or DIELOG($outLog, "footLoop_2fixBAM.pl: Failed to read from filetype=$filetype, file=$LCY$file$N: $!\n");
	}
	else {
		($total_line) = `wc -l $file` =~ /^\s*(\d+)/;
		open ($checkfileIn, "<", $file) or DIELOG($outLog, "footLoop_2fixBAM.pl: Failed to read from filetype=$filetype, file=$LCY$file$N: $!\n") 
	}
	...
	while (my $line = <$checkfileIn>) {
		...
		if ($type eq "BAM") {
			...
			if (defined $currseq and $currseq =~ /^[ACTGUN]+$/i) {
				...
				DIELOG($outLog, __LINE__ .  "footLoop_2fixBAM.pl: linecount=$linecount file=$file type=$type, check=$check, check_file failed (does not seem to be a seq file\n\t-> fasta file start with non-header!\n\n") if $check == 0;
				DIELOG($outLog, __LINE__ .  "footLoop_2fixBAM.pl: linecount=$linecount file=$file type=$type check=$check line=$line corruped fasta file!\n\t-> multiple headers in a row\n\n") if $check % 2 != 0;
				undef $currseq;
			}
	...
	DIELOG($outLog, __LINE__ .  "footLoop_2fixBAM.pl: linecount=$linecount file=$file type=$type, check=$check, check_file failed (does not seem to be a BAM file\n\t-> there's no read with more than 10 collumns!\n") if $check == 0;
}
		...
		for (my $i = 0; $i < @line; $i++) {
			...
			if ($line =~ /footLoop.pl,geneIndexFile/) {
				...
				foreach my $line (@line) {
					...
					$gene = uc($gene);
					$genez->{$gene} = $length;
					LOG($outLog, date() . "\t\tfootLoop_2fixBAM.pl: gene $gene = $length bp\n");
				}
			}
...
#			}
#			last if $line =~ /footLoop_2fixBAM.pl/;
		}
	}
...
sub get_bad_region {
	...
	for (my $i = $seqborder0; $i < $seqborder1; $i++) {
		...
		for (my $j = $i; $j < $seqborder1; $j++) {
			DIELOG($outLog, "footLoop_2fixBAM.pl: Undefined ref2 at j=$j\n") if not defined $ref2->[$j];
			$reftemp .= $ref2->[$j];
			DIELOG($outLog, "footLoop_2fixBAM.pl: Undefined seq2 at j=$j\n") if not defined $seq2->[$j];
			$seqtemp .= $seq2->[$j];
...
sub getConv {
	...
	DIELOG($outLog, "footLoop_2fixBAM.pl: X=$X, H=$H, Z=$Z, U=$u, x=$x, h=$h, z=$z, u=$u, dot=$dot, length=$length, length2=$length2\n\n") if $length ne $length2;
...
sub check_chr_in_BAM {
	...
	while (my $line = <$in2>) {
		...
		next if $strand eq 4;
		DIELOG($outLog, "footLoop_2fixBAM.pl: Can't find gene $chr in $seqFile!\n") if not defined $refs{$chr};
		next;
	}
}
__END__
