...
sub main {
	...
	#DEBUG HERE
	#return if $faFile !~ /T7_INIT_VR_16/;

	LOG($outLog, "
-----------------
$YW footPeakAddon2.pm $version_small $N
---------
");
	...
	for (my $h = 0; $h < 4; $h++) {
		...
		$files{$peakFile} = 1;
#		die "GOOD2 \n";
#		next unless $peakFile =~ /T7_INIT_VR_16_Pos.+CG/;
