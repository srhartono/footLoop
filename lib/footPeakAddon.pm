           ...
           df3\$x = as.numeric(as.character(df3\$x))
            ...
            p3 = ggplot(df3,aes(x,y)) +
               ...
               geom_rect(data=df4,aes(xmin=xmin,xmax=xmax,ymin=ymin,
												 ...
												 ),size=4,fill=rgb(0,0,0,alpha=0)) + #color=rgb(1,0,0,alpha=1)) +
...
#					coord_fixed() +
            ...
            ratio1 = dim(df)[1]*16 / (dim(df)[1]*16 + 500 + 425)
            ratio2 = 500           / (dim(df)[1]*16 + 500 + 425)
			";
			if (-e $curr_cluster_file and -s $curr_cluster_file > 0) {
				$Rscript .= "
           ratio3 = 425           / (dim(df)[1]*16 + 500 + 425)
		png(\"$pngout\",width=dim(df)[2]*2,height=dim(df)[1]*16 + 500 + 425)
