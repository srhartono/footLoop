...
use vars qw($opt_v $opt_g $opt_p $opt_d $opt_s $opt_k $opt_K $opt_n $opt_h $opt_t $opt_w $opt_L $opt_l $opt_o $opt_A $opt_G $opt_Z $opt_O);
getopts("vg:p:d:s:k:K:n:ht:w:l:A:o:G:L:ZO:");
...
for (my $i = 0; $i < @origFile; $i++) {
	#next if $origFile[$i] !~ /T7_INIT_VR_6_DESCT7MIX_TX_RH1_(Pos|Neg).filtered.gz/;
	#push(@neworigFile, $origFile[$i]);
	...
	$genes{uc($gene)} ++;
	LOG($outLog, "Exmaple: $LCY$origFile[$i]\t$gene\t$strand$N\n\n") if $i eq 0;
}
...
=comment
		my $comm = [$gene, $strand, $val, $name, $window, $threshold, $check];
		#DIELOG($outLog, "$LGN$linecount$N $LCY$line$N\n");
		$Q->enqueue($comm);
		if ($Q->pending == 60) {
			LOG($outLog, date() . "-> Done $linecount\n","NA") if $linecount % 300 == 0;
			$Q->end();
			$R = new Thread::Queue;
	...
	#next;
	#DIELOG($outLog, "Exit after peakFile\n");
	...
	LOG($outLog, "\n\n" . date() . "\n\n","NA");
	my $peakFilez = "$outDir/.CALL/$peakFilename\_$window\_$threshold\_CG.PEAK";
#   my ($input1, $faFile, $mygene, $minDis, $outDir, $minLen, $SEQ, $outLog) = @_;
	my $footPeakres = footPeakAddon::main(($peakFilez, $seqFile, $gene, $minDis, $outDir, $minLen, $SEQ, $i, scalar(@origFile), $version_small, $outLog));
	DIELOG($outLog, "\n" . date() . "Failed footpeak\n") if defined $footPeakres and $footPeakres eq -1;
}
=cut
#DIELOG($outLog, "Exit after ALL peakFile\n");
#system("/bin/cp $opts->{origDir}/footPeak_logFile.txt $outDir/");
#COMMENTCUT
#	my $t1 = Benchmark->new();
