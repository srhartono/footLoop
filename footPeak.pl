...
my %opts = %{parse_logFile($logFile)};
my $faFile = $opts{g};
my $indexFile = $opts{i};
my $origFolder = $opts{o};
my $x = $opts{x};
my $y = $opts{y};
my $min = defined $opt_d ? $opt_d : 100;
my $groupsize = defined $opt_s ? $opt_s : 200;
my $dist = defined $opt_k ? $opt_k : 50;
my $outDir = defined $opt_n ? $opt_n : $origFolder;
...
die $usage . "faFile=$faFile, indexFile=$indexFile, origFolder=$origFolder, outdir=$outDir\n" unless defined $faFile and defined $indexFile and defined $origFolder and -e $faFile and -e $indexFile and -e $origFolder and defined $outDir;
...
for (my $i = 0; $i < @origFile; $i++) {
	...
	$peakFilename =~ s/.orig$//;
	my ($gene) = $peakFilename =~ /^(\w+)_(Pos|Neg|Unk)$/; $gene = uc($gene);
	print "FILENAME=$peakFilename, GENE=$gene\n";
	die "Died cannot get gene from peakfile $peakFile\n" unless defined $gene;

	my $total = @{$seq{$gene}}; 
	my (%data, %end, %bad);
	my ($totalPeak, $linecount, $peakcount) = (0,0,0);
	my %final; my %group;
...
sub parse_logFile {
	...
	my %opts = ("x" => 0, "y" => 0, "g" => "", "i" => "", "q" => 0, "r" => "", "L" => 0);
	foreach my $line (@head[0..@head-1]) {
		next if $line !~ /^Run script :/;
		my ($opts) = $line =~ /^Run script[ ]+:(.+)$/;
		my @opts = split(" ", $opts);
...
#			print "-$opt: $val\n";
	...
	$opts{g} = $faFile;
	die "Fasta file $faFile does not exist!\n" if not -e $faFile;
	return(\%opts);
}
sub parse_index {
...
die "Please run footloop.pl first! ($footFolder/logFile.txt does not exists)\n" if not -e "$footFolder/logFile.txt";
