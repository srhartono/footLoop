...
use vars qw($opt_w $opt_g $opt_G $opt_v $opt_n $opt_r $opt_R $opt_B $opt_c $opt_E); #v $opt_x $opt_R $opt_c $opt_t $opt_n);
getopts("n:vg:w:G:r:R:B:cE:");
...
-E make graph for reads in this separately
";
...
my $excludeFile = $opt_E;
#my %exclude;
#if (defined $opt_E) {
#	my ($file) = $opt_E;
#	my $in1;
#	if ($file =~ /.gz$/) {
#		open ($in1, "zcat $file|") or die "Failed to read from -E $file: $!\n";
#	}
#	else {
#		open ($in1, "<", $file) or die "Failed to read from -E $file: $!\n";
#	}
#	while (my $line = <$in1>) {
#		chomp($line);
#		my @arr = split("\t", $line);
...
#die "Excluding $excluded reads\n";

main($opt_n, $excludeFile);
sub main {
	my ($resDir) = @_;
...
","NA");
			...
			my $maxindice = (defined($excludeFile) and -e $excludeFile) ? 2 : 1;
			for (my $indice = 0; $indice < $maxindice; $indice++) {
...
#library(Cairo)
				...
				$pngout = "$resDir/PNG/$pngoutDir/$currFilename.$indice.$flag.png";
				$pdfout = "$resDir/PDF/$pdfoutDir/$currFilename.$indice.$flag.pdf";
				$lenpdfout = "$resDir/PDF/$pngoutDir$currFilename\_length.$indice.pdf";
				my $isExclude = $indice;
			...
			#		$Rscript .= "png(type=\"cairo\",\"$pngout\",1000,1000)\nplot(NA,xlim=c(1,100),ylim=c(1,100),xlab=NA,ylab=NA,bty=\"n\")\ntext(50,50,cex=3,labels=c(\"$currFilename\n\nPEAK = $totpeak / $totread\"))\ndev.off()\n";
					...
					close $incurrFile;
					close $outcurrFileID;
					my ($R) = Rscript($currFile, $bedFile, $curr_cluster_file, $totpeak, $totnopk, $pngout, $pdfout, $lenpdfout, $resDir, $isExclude, $excludeFile);
	...
	#					print "\n\n----------------- $LCY$currFile$N IS A PEAK FILE R = $currFile.PNG.R -------------- \n\n";
				...
				open (my $outRscriptPNG, ">", "$currFile.$isExclude.PNG.R") or (LOG($outLog, date() . "Failed to write R script into $currFile.$isExclude.PNG.R: $!\n") and print $outLog $Rscript and next);
				print $outRscriptPNG $RscriptPNG;
				$Rscripts{"$currFile.$isExclude.PNG.R"}{summary} = $summary;
				$Rscripts{"$currFile.$isExclude.PNG.R"}{runR} = (defined $opt_c and $flag =~ /^(NOPK_C|PEAK_C|NOPK_TEMP_C|PEAK_TEMP_C)$/) ? 1 : $flag =~ /(ALL|RCONV|_C)/ ? 0 : 1;
				$Rscripts{"$currFile.$isExclude.PNG.R"}{runType} = $flag;
				close $outRscriptPNG;
	
				open (my $outRscriptPDF, ">", "$currFile.$isExclude.PDF.R") or (LOG($outLog, date() . "Failed to write R script into $currFile.$isExclude.PDF.R: $!\n") and print $outLog $Rscript and next);
				print $outRscriptPDF $RscriptPDF;
				close $outRscriptPDF;
	
				if ($currFile =~ /\.NOPK\./) {
					open (my $outRscriptPNG_nopk_ALL, ">", "$currFile.$isExclude.PNG_nopk_ALL.R") or (LOG($outLog, date() . "Failed to write R script into $currFile.$isExclude.PNG_nopk_ALL.R: $!\n") and print $outLog $Rscript and next);
					print $outRscriptPNG_nopk_ALL $RscriptPNG_nopk_ALL;
					$Rscripts{"$currFile.$isExclude.PNG_nopk_ALL.R"}{summary} = $summary;
					$Rscripts{"$currFile.$isExclude.PNG_nopk_ALL.R"}{runR} = 0;
					$Rscripts{"$currFile.$isExclude.PNG_nopk_ALL.R"}{runType} = $flag . "_ALL";
					close $outRscriptPNG_nopk_ALL;
	
					open (my $outRscriptPDF_nopk_ALL, ">", "$currFile.$isExclude.PDF_nopk_ALL.R") or (LOG($outLog, date() . "Failed to write R script into $currFile.$isExclude.PDF_nopk_ALL.R: $!\n") and print $outLog $Rscript and next);
...
sub Rscript {
	my ($currFile, $bedFile, $curr_cluster_file, $totpeak, $totnopk, $pngout, $pdfout, $lenpdfout, $resDir, $isExclude, $excludeFile) = @_;
	my $currFileID = $currFile . ".id";
	my $R;
	...
	my $plotminReads = 4;
	$isExclude = -1 if not defined $isExclude;
...
if (defined $excludeFile and -e $excludeFile) {
	$R->{readTable} .= "
	library(\"data.table\")
	exclude = fread(\"$excludeFile\",sep=\"\\t\")
	exclude\$V1 = paste(exclude\$V5,\".\",exclude\$V1,sep=\"\")
	print(dim(exclude))
	#df1 = df[df\$V1 \%in\% exclude\$V1,]
	#df2 = df[!df\$V1 \%in\% exclude\$V1,]
	";
}
...
if (dim(exclude)[1] > 0) {
	if (isExclude == 0) {
		print(\"Good: df Before:\")
		print(dim(df))
		#df = df[!df\$V1 \%in\% exclude\$V1,]
		df\$df0 = apply(df[,seq(200,400)],1,function(x){return(length(x[x == 0]))})
		df = df[df\$df0 < 5,]
		df = subset(df,select=-df0)
		print(\"Good: df After\")
		print(dim(df))
	} else if (isExclude == 1) {
		print(\"Excluded: df Before:\")
		print(dim(df))
		#df = df[df\$V1 \%in\% exclude\$V1,]
		df\$df0 = apply(df[,seq(200,400)],1,function(x){return(length(x[x == 0]))})
		df = df[df\$df0 >= 5,]
		df = subset(df,select=-df0)
		print(\"Excluded: df After\")
		print(dim(df))
	}
}

df.id = df.id[df.id\$V1 \%in\% df\$V1,]
...
print(\"clust Before:\")
print(dim(clust))
clust = clust[clust\$id \%in\% df\$id,] # ADD
print(\"clust After:\")
print(dim(clust))
